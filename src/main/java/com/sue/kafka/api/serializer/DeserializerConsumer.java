        ...
        try {
            ...
            while(true){
                ...
                poll.partitions().forEach(topicPartition -> {
                    ...
                    List<ConsumerRecord<String, User>> records = poll.records(topicPartition);
                    ...
                    for(int i = 0;i<size;i++){
                        //实际数据
                        ConsumerRecord<String, User> consumerRecord = records.get(i);
                        User value = consumerRecord.value();
                        long offset = consumerRecord.offset();
                        long commitOffset = offset+1;
                        System.out.println(String.format("获取实际消息value:%s,消息offset:%s,提交offset:%s",value.getName(),offset,commitOffset));
                    }
                });
            }
        }finally {
